<template>
  <div class="model-lab-container">
    <!-- 顶部导航 -->
    <a-card :bordered="false" class="lab-header" :body-style="{ padding: isMobile ? '12px' : '24px' }">
      <div class="header-main" :class="{ 'mobile': isMobile }">
        <div class="title-section">
          <ExperimentOutlined class="lab-icon" />
          <div class="title-content">
            <h1>QuantLab Pro</h1>
            <span class="subtitle" v-if="!isMobile">专业级量化基金研究平台</span>
          </div>
        </div>
        <a-radio-group v-model:value="activeModule" button-style="solid" :size="isMobile ? 'small' : 'large'" class="tab-nav">
          <a-radio-button value="factor">
            <ApartmentOutlined /> {{ isMobile ? '因子' : '多因子研究' }}
          </a-radio-button>
          <a-radio-button value="smartbeta">
            <ThunderboltOutlined /> {{ isMobile ? 'Smart' : 'Smart Beta' }}
          </a-radio-button>
          <a-radio-button value="ml">
            <RobotOutlined /> {{ isMobile ? 'AI/ML' : 'AI/ML策略' }}
          </a-radio-button>
          <a-radio-button value="allocation">
            <PieChartOutlined /> {{ isMobile ? '配置' : '资产配置' }}
          </a-radio-button>
          <a-radio-button value="risk">
            <SafetyOutlined /> {{ isMobile ? '风险' : '风险管理' }}
          </a-radio-button>
        </a-radio-group>
      </div>
    </a-card>
