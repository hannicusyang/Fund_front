<template>
  <div class="fund-model-container">
    <!-- 顶部导航 -->
    <a-card :bordered="false" class="header-card" :body-style="{ padding: isMobile ? '12px' : '24px' }">
      <div class="header-content" :class="{ 'mobile': isMobile }">
        <div class="title-section">
          <FundOutlined class="title-icon" />
          <div class="title-text">
            <h1>基金投资模型实验室</h1>
            <span class="subtitle" v-if="!isMobile">基于公募基金的量化投资组合构建与回测</span>
          </div>
        </div>
        <a-radio-group v-model:value="activeTab" button-style="solid" :size="isMobile ? 'small' : 'large'" class="tab-nav">
          <a-radio-button value="screening">
            <FilterOutlined /> {{ isMobile ? '筛选' : '基金筛选' }}
          </a-radio-button>
          <a-radio-button value="analysis">
            <BarChartOutlined /> {{ isMobile ? '分析' : '指标分析' }}
          </a-radio-button>
          <a-radio-button value="portfolio">
            <PieChartOutlined /> {{ isMobile ? '组合' : '组合构建' }}
          </a-radio-button>
          <a-radio-button value="backtest">
            <LineChartOutlined /> {{ isMobile ? '回测' : '回测验证' }}
          </a-radio-button>
        </a-radio-group>
      </div>
    </a-card>
